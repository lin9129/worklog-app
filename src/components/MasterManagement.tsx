...
export default function MasterManagement({ masterData }: Props) {
    ...
    const renderPartList = () => {
        const grouped = masterData.parts.reduce((acc: any, part: any) => {
            const prodName = part.product?.name || '不明な商品'
            if (!acc[prodName]) acc[prodName] = []
            acc[prodName].push(part)
            return acc
        }, {})

        return Object.entries(grouped).map(([prodName, parts]: [string, any]) => (
            <div key={prodName} style={{ marginBottom: '1.5rem' }}>
                <h4 style={{ fontSize: '0.9rem', opacity: 0.7, marginBottom: '0.5rem', borderLeft: '3px solid var(--primary)', paddingLeft: '0.5rem' }}>{prodName}</h4>
